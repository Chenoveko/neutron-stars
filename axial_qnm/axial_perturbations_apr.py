...
p_anchor = 1.4717674576446888e+35 # anchor point at 1.4 solar masses for central pressure
...
for p_c in p_central_array_geo_up:
    it_up += 1
    # Solve TOV
    r_apr, p_apr, m_apr, nu_apr, _ = solve_tov_eos(p_c, rho_pchip_geo_apr)
    # Interpolation -> rho(r), p(r), m(r), nu(r)
    rho_apr = array([rho_pchip_geo_apr(p) for p in p_apr])
    p_fun = pchip(r_apr, p_apr, extrapolate=False)
    m_fun = pchip(r_apr, m_apr, extrapolate=False)
    nu_fun = pchip(r_apr, nu_apr, extrapolate=False)
    rho_fun = pchip(r_apr, rho_apr, extrapolate=False)
    # TOV parameters
    R = r_apr[-1]
    M = m_apr[-1]
    r0 = r_apr[0]
    ...
    if it_up == 1:
        f_mesh = linspace(8.1e3, 8.3e3, 5)
        tau_mesh = linspace(27.5e-6, 31.5e-6, 5)
        ...
        print(f"Minimum at first iteration APR → f = {f_best:.3f} KHz | τ = {tau_best:.3f} µs | log10(p_c [cgs]) = {log10(pressure_geo_to_cgs(p_c)):.6f}")
        # ================================
        # Heatmap 2D
        # ================================
        ...
        ax_heat2.set_title(rf'Heatmap matching APR, iteration {it_up}')
        ...
        ax_heat3.set_title(rf'Matching 3D surface APR, iteration {it_up}')
        ...
        print(f"Müller APR → f = {f_muller:.6f} kHz | τ = {tau_muller:.6f} µs | it = {res.iterations} | log10(p_c [cgs]) = {log10(pressure_geo_to_cgs(p_c)):.6f}")
        f_apr_up.append(f_muller)
        tau_apr_up.append(tau_muller)
    elif it_up == 2:
        f_mesh = linspace(8.1e3, 8.3e3, 5)
        tau_mesh = linspace(27.5e-6, 31.5e-6, 5)
        F, TAU = meshgrid(f_mesh, tau_mesh)
        ...
        f_best = F[idx] / 1e3
        tau_best = TAU[idx] * 1e6
        print(f"Minimum at second iteration APR → f = {f_best:.3f} KHz | τ = {tau_best:.3f} µs | log10(p_c [cgs]) = {log10(pressure_geo_to_cgs(p_c)):.6f}")
        # ================================
        # Heatmap 2D
        # ================================
        fig_heat2, ax_heat2 = plt.subplots(figsize=(7.5, 4.5))
        ...
        (w1, w2, w3), (f0, tau0), seeds = muller_seed_from_extrapolation(
            p_central_array_geo_up[:len(f_apr_up)],
            array(f_apr_up) * 1e3,  # kHz -> Hz si tus listas están en kHz
            array(tau_apr_up) * 1e-6,  # us -> s
            p_c,
            c_cgs,
            f_match=f_match,
            df_rel=1e-4,
...
for p_c in p_central_array_geo_down:
    ...
    if it_down == 1:
        f_mesh = linspace(8.1e3, 8.3e3, 5)
        tau_mesh = linspace(27.5e-6, 30.0e-6, 5)
        F, TAU = meshgrid(f_mesh, tau_mesh)
        OMEGA = (2.0 * pi * F - 1j / TAU) / c_cgs
        # Evaluate abs(matching function) in meshgrid
        from joblib import Parallel, delayed
        ...
        ax_heat2.set_xlabel('f [KHz]')
        ax_heat2.set_ylabel(r'$\tau$ [$\mu$s]')
        ax_heat2.set_title(rf'Heatmap matching APR, iteration {it_down}')
        ax_heat2.plot(f_best, tau_best, 'ro', ms=4, label='Mínimo')
        ax_heat2.legend(loc='upper right', frameon=False)
        # ================================
        # Heatmap 3D
        ...
        ax_heat3.set_ylabel(r'$\tau$ [$\mu$s]')
        ax_heat3.set_zlabel(r'$\log_{10}|f_{\mathrm{match}}|$')
        ax_heat3.set_title(rf'Matching 3D surface APR, iteration {it_down}')
        ax_heat3.scatter(f_best, tau_best, f_meshgrid_log10[idx], color='red', s=10, label='Mínimo')
        ax_heat3.legend()
        plt.show()
        # Muller search
        ...
        tau_muller = -1 / (res.root.imag * c_cgs) * 1e6
        print(f"Müller APR → f = {f_muller:.6f} kHz | τ = {tau_muller:.6f} µs | it = {res.iterations} | log10(p_c [cgs]) = {log10(pressure_geo_to_cgs(p_c)):.6f}")
        f_apr_down.append(f_muller)
        tau_apr_down.append(tau_muller)
    elif it_down == 2:
        f_mesh = linspace(8.1e3, 8.3e3, 5)
        tau_mesh = linspace(27.5e-6, 30.0e-6, 5)
        F, TAU = meshgrid(f_mesh, tau_mesh)
        ...
        (w1, w2, w3), (f0, tau0), seeds = muller_seed_from_extrapolation(
            p_central_array_geo_down[:len(f_apr_down)][::-1],
            array(f_apr_down[::-1]) * 1e3,
            array(tau_apr_down[::-1]) * 1e-6,
            p_c,
            c_cgs,
            f_match=f_match,
            df_rel=1e-4,
        ...
        try:
            ...
            f_apr_down.append(nan)
            tau_apr_down.append(nan)
            continue
        f_muller = res.root.real / (2 * pi) * c_cgs / 1e3
        tau_muller = -1 / (res.root.imag * c_cgs) * 1e6
        print(f"Müller APR → f = {f_muller:.6f} kHz | τ = {tau_muller:.6f} µs | it = {res.iterations} | log10(p_c [cgs]) = {log10(pressure_geo_to_cgs(p_c)):.6f}")
...
f_down_sorted = array(f_apr_down)[::-1]
tau_down_sorted = array(tau_apr_down)[::-1]
# Remove duplicate anchor point
p_down_sorted = p_down_sorted[:-1]
f_down_sorted = f_down_sorted[:-1]
tau_down_sorted = tau_down_sorted[:-1]
# Concatenate
p_total_apr = concatenate((p_down_sorted, p_central_array_geo_up))
f_total_apr = concatenate((f_down_sorted, array(f_apr_up)))
tau_total_apr = concatenate((tau_down_sorted, array(tau_apr_up)))
# Make xlabel
log_p_total_apr = log10(pressure_geo_to_cgs(p_total_apr))
# Save results
savez("apr_results.npz",log_p=log_p_total_apr,f=f_total_apr,tau=tau_total_apr)

# ==========Plot 1: Frequency vs Central Pressure==========#
fig_f, ax_f = plt.subplots(figsize=(7.5, 4.5))
ax_f.plot(log_p_total_apr, array(f_total_apr), color='goldenrod', linewidth=1.5, label="APR")
